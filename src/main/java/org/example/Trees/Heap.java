...
public abstract class Heap<T extends Comparable<T>> implements Tree<T> , Serializable {
    ...
    public List<T> inorderTraversal() {
        ...
        if (!heap.isEmpty()) {
            inorderTraversalHelper(0, result);  // Start at the root (index 0)
        }
        return result;
    }

    private void inorderTraversalHelper(int index, List<T> result) {
        if (index >= heap.size()) {
            return;
        }
        ...
        if (leftChildIndex < heap.size()) {
            inorderTraversalHelper(leftChildIndex, result);
        }
        ...
        if (rightChildIndex < heap.size()) {
            inorderTraversalHelper(rightChildIndex, result);
        }
    }
